...
function AboutPage({ go, onHome }) {
  ...
  return <div className="about-page">
    ...
    <main>
      ...
      <section className="about-detail experience-detail section" id="experience"><div className="section-kicker">WORK EXPERIENCE</div><div className="experience-lead"><h2>A career spanning<br/><em>many lenses.</em></h2><p>A career spanning HR leadership, UX research, product design, and academic publishing — each chapter deepening a systems-level perspective on how people interact with organisations and products.</p></div><div className="experience-timeline">{experiences.map(([date, role, company, copy, tags]) => <article key={role}><span className="timeline-date">{date}</span><div><h2>{role}</h2><p className="school-line">{company}</p><div className="experience-copy">{(Array.isArray(copy) ? copy : [copy]).map(paragraph => <p key={paragraph}>{paragraph}</p>)}</div><div className="experience-tags">{tags.split(' · ').map(tag => <span key={tag}>{tag}</span>)}</div></div></article>)}</div></section>
